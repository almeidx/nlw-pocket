...
export async function getWeekSummary() {
	...
	const result = await db
		.with(goalsCreatedUpToThisWeek, goalsCompletedInWeek, goalsCompletedByWeekDay)
		.select({
			completed: sql`(SELECT COUNT(*) FROM ${goalsCompletedInWeek})`.mapWith(Number).as("completed"),
			total: sql`(SELECT SUM(${goalsCreatedUpToThisWeek.desiredWeeklyFrequency}) FROM ${goalsCreatedUpToThisWeek})`
				.mapWith(Number)
				.as("total"),
	...
	return result[0] ?? { completed: 0, total: 0, goalsPerDay: {} };
}
